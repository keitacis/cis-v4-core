#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CIS v4 TradingView monthly refresh v3 partial coverage patch R6.
...
CORE_MARKER_START = "# --- CIS R6 TV covered_partial compatibility START ---"
CORE_MARKER_END = "# --- CIS R6 TV covered_partial compatibility END ---"
MASTER_MARKER_START = "# --- CIS R6 TV covered_partial master update START ---"
MASTER_MARKER_END = "# --- CIS R6 TV covered_partial master update END ---"
...
CORE_BLOCK = r'''
# --- CIS R6 TV covered_partial compatibility START ---
# R6: Treat TradingView partial coverage as a first-class state.
...
def _tv_r6_is_covered(self: TVSnapshot) -> bool:
    return self.coverage_status in {"covered", "covered_partial"}

def _tv_r6_is_tv_ok(self: TVSnapshot) -> bool:
...
TVSnapshot.is_covered = property(_tv_r6_is_covered)  # type: ignore[assignment]
TVSnapshot.is_tv_ok = property(_tv_r6_is_tv_ok)  # type: ignore[assignment]
...
def tv_distribution_dict(tv: Optional[TVSnapshot]) -> Dict[str, int]:  # type: ignore[override]
    ...
    return out
# --- CIS R6 TV covered_partial compatibility END ---
'''

MASTER_BLOCK = r'''
# --- CIS R6 TV covered_partial master update START ---
...
# --- CIS R6 TV covered_partial master update END ---
'''
...
def patch_buy_alert() -> None:
    text = read(BUY)
    text = text.replace('"covered": "アナリスト予想あり", "no_coverage": "カバレッジなし",', '"covered": "アナリスト予想あり", "covered_partial": "アナリスト予想あり（一部取得）", "no_coverage": "カバレッジなし",')
    text = text.replace('r.get("tv_coverage_status") == "covered"', 'r.get("tv_coverage_status") in {"covered", "covered_partial"}')
    text = text.replace('米国株TradingView covered', '米国株TradingView 予想あり')
    if 'covered_partial' not in text:
        raise RuntimeError("covered_partial not injected into buy_alert")
    write(BUY, text)
...
def runtime_tests() -> None:
    ...
    buy_text = read(BUY)
    if '"covered_partial": "アナリスト予想あり（一部取得）"' not in buy_text:
        raise RuntimeError("buy_alert covered_partial label missing")
...
def main() -> int:
    ...
    runtime_tests()
    clean_pycache()
    print("R6 patch applied and safety tests passed.")
    return 0

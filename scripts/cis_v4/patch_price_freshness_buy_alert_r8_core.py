#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CIS v4 price freshness + Buy Alert UX patch R8 core-only.
...
def main() -> int:
    """Apply core/report/data changes only.

    R8 intentionally does not write .github/workflows/* because GitHub Actions
    cannot push workflow-file changes with the default GitHub App token. The
    workflow cron files are supplied separately for a user-authenticated commit
    via GitHub Desktop.
    """
    required = [
        SCRIPTS / "cis_core.py",
    ...
    patch_settings()
    print("CIS price freshness + Buy Alert UX patch R8 core-only applied. Workflow cron files were not modified.")
    return 0

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply CIS v4 R12.2 degradation overwrite guard.

R12.2 prevents a known failure mode: a healthy latest report can be overwritten
...
def patch_core() -> bool:
    ...
    new_text, n = re.subn(pattern, lambda _m: R12_CORE_CODE, text, count=1)
    if n != 1:
...
R12.2 は、正常な latest レポートがある状態で、後続実行が全銘柄 `price_stale` / 価格未更新を返した場合に、その悪い結果で latest 本文・latest JSON を上書きしないためのガードです。
...
def main() -> int:
    ...
    print("R12.2 patch applied. Changed:")
    for item in changed:
        print("-", item)
    return 0
